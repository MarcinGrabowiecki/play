...
if (1 == 2) {
  ...
  fileref.setAttribute("src", "http://run.plnkr.co/kK87V233ICC6KHnB/script.js?dupa")
...
function duzoPowtorzen2(nr){
  for(i=0; i<nr.length-3;i++){
    var p=nr.substring(i,i+3)
    var reg=new RegExp(p+".*"+p+".*"+p)
    if(reg.test(nr)) return " 2Powtórzenia"
  }
  return ""
}
...
_.each(nums, function(nr) {
  var r=maloCyfr(nr)+sekwencja(nr)+duzoPowtorzen(nr)+duzoPowtorzen2(nr)+f123(nr)+f000(nr);
  if(r.length>1) { alert(nr+" "+r); clearInterval(br); return false }
  return true;
})
}
